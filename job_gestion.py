...
class JobApplicationApp:
    def __init__(self, root):
        ...
        self.home_frame = tk.Frame(self.root, bg='#333333', width=1400, height=800)
        self.home_frame.pack_propagate(False)
        self.add_frame = tk.Frame(self.root, bg='#333333', width=1400, height=800)
        ...
        # Assurer le redimensionnement correct sur Windows
        self.root.update_idletasks()
        self.root.geometry(self.root.geometry())  # Mettre à jour la géométrie pour la taille actuelle de la fenêtre

    def build_home_page(self):
        # Ajouter le logo si disponible
        if self.logo_photo:
            logo_label = tk.Label(self.home_frame, image=self.logo_photo, bg='#333333')
            logo_label.pack(pady=10)

        # Titre de la page d'accueil
        title_label = tk.Label(self.home_frame, text="Vos Candidatures", font=("Helvetica", 14), bg='#333333', fg='#ffffff')
        ...
        self.application_list_frame = tk.Frame(self.home_frame, bg='#333333', width=1200, height=500)
        self.application_list_frame.pack(pady=10, padx=10, fill='both', expand=True)
        self.application_list_frame.pack_propagate(False)
        self.application_list_frame.grid_propagate(False)

        # Configurer la grille pour prendre tout l'espace disponible
        self.application_list_frame.columnconfigure(0, weight=1)  # #
        self.application_list_frame.columnconfigure(1, weight=3)  # Entreprise
        self.application_list_frame.columnconfigure(2, weight=3)  # Poste
        self.application_list_frame.columnconfigure(3, weight=2)  # Date
        self.application_list_frame.columnconfigure(4, weight=2)  # Statut
        self.application_list_frame.columnconfigure(5, weight=3)  # Actions

        # Navigation entre les pages
        self.navigation_frame = tk.Frame(self.home_frame, bg='#333333', width=1200, height=50)
        self.navigation_frame.pack(pady=20)

        # Ajouter une colonne pour séparer les boutons et éviter des soucis de placement
        ...
        self.prev_page_btn = tk.Button(self.navigation_frame, text="<<", command=self.go_to_first_page, state=tk.DISABLED,
                                    relief=tk.RAISED, bd=2, bg='#e0e0e0', fg='#000000', cursor="arrow")
        self.prev_page_btn.grid(row=0, column=1, padx=10, pady=10)

        self.prev_btn = tk.Button(self.navigation_frame, text="<", command=self.go_to_prev_page, state=tk.DISABLED,
                                relief=tk.RAISED, bd=2, bg='#e0e0e0', fg='#000000', cursor="arrow")
        self.prev_btn.grid(row=0, column=2, padx=10, pady=10)
        ...
        self.next_btn = tk.Button(self.navigation_frame, text=">", command=self.go_to_next_page,
                                relief=tk.RAISED, bd=2, bg='#e0e0e0', fg='#000000', cursor="arrow")
        self.next_btn.grid(row=0, column=4, padx=10, pady=10)

        self.next_page_btn = tk.Button(self.navigation_frame, text=">>", command=self.go_to_last_page,
                                    relief=tk.RAISED, bd=2, bg='#e0e0e0', fg='#000000', cursor="arrow")
        self.next_page_btn.grid(row=0, column=5, padx=10, pady=10)

        # Mettre à jour la liste des candidatures
        self.update_application_list()

        # Bouton pour ajouter une nouvelle candidature
        add_btn = tk.Button(self.home_frame, text="Nouvelle candidature", command=self.switch_to_add_page, bg='#ffffff', fg='#000000', relief=tk.FLAT, highlightthickness=0, cursor="arrow")
